...
def accuracy1(output, target, topk=(1,)):
    ...
    with torch.no_grad():
        ...
        _, pred = output[0].unsqueeze(0).topk(maxk, 1, True, True)
        # output_mean = output.mean(dim=0, keepdim=True)  # 첫 번째 dimension 평균 계산
        # _, pred = output_mean.topk(maxk, 1, True, True)  # Top-k 추출
        pred = pred.t()
        correct = pred.eq(target.view(1, -1).expand_as(pred))
